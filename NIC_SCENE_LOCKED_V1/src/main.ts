...
async function playNICScene() {
  ...
  try {
    ...
    await sleep(1100, runId);

    /*
      Beat 1 — Nick opens the birthday mission.
    */
    ...
    showDialogue(
      "NICK WILDE",
      "Agent Nishu... birthday mission officially starts here.",
    );
    ...
    */
    ...
    /*
      Beat 3 — Nick reveals checkpoint #1.
    */
    setClip("Nick", "Pointing", true);

    showDialogue(
      "NICK WILDE",
      "First checkpoint: Strawberries by Jagro.",
    );
    ...
    */
    ...
    /*
      Exit beat.
      The Walk animations themselves stay clip-local; movement through
      the environment happens on the PLACEMENT roots.
    */
    ...
    while (true) {
      ...
      const elapsed =
        performance.now() - walkStart;

      const t =
        Math.min(
          1,
          elapsed / walkDuration,
        );
    ...
    await sleep(2600, runId);

    transitionCard.classList.add("hidden");

    /*
      Return editor state to the approved starting shot.
    */
    applyIdleStaging();
    ...
    console.log("[NIC] cinematic complete");
  } catch (error) {
    if (
      error instanceof Error &&
      error.message === "CINEMATIC_CANCELLED"
    ) {
      return;
    }
